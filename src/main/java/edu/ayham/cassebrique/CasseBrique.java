...
public class CasseBrique extends Canvas {
    ...
    public void demarrer() throws InterruptedException {
        ...
        while(true) {
            ...
            yBalle +=vitesseVerticalalBalle;

            dessin.setColor(Color.RED);
            dessin.fillOval(xBalle,yBalle, diametreBalle,diametreBalle);

            dessin.setColor(Color.WHITE);
            dessin.fillOval(xBalle + decalageReflet,yBalle + decalageReflet, decalageReflet,diametreRefletBalle);

            //mouvement balle
            if (xBalle<0 || xBalle>500 -diametreBalle){vitesseHorizontalBalle*=-1;
                //vitesseHorizontalBalle=*vitesseHorizontalBalle*-1
            }
            if (yBalle<0 || yBalle>500-diametreBalle){vitesseVerticalalBalle*=-1;
            }
